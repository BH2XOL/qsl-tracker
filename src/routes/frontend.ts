...
import { countCards, queryCards, countBySentStatus, countByRcvdStatus } from "../lib/db";
...
function methodBadge(method: string): string {
  if (!method) return "";
  const label: Record<string, string> = { Bureau: "卡片局", Direct: "直邮", "电子": "电子", "眼球QSO": "眼球QSO" };
...
<body>
  ...
  <main class="main">
    ...
    <div class="stat-grid">
      ...
      <div class="stat-card"><div class="stat-label">发卡总计</div><div class="stat-value">${sentCount + pendingSent}</div><div class="stat-sub">卡片局 / 直邮 / 电子 / 眼球QSO</div></div>
    ...
    <div class="search-bar">
      <div class="search-field" style="flex:2;min-width:140px;">
        <label>呼号</label>
        <input type="text" placeholder="搜索呼号…" id="callSearch" value="${esc(callF || "")}" maxlength="10" style="text-transform:uppercase;">
      </div>
  ...
  <script>
    ...
    function search() {
      ...
      var c=document.getElementById('callSearch').value.trim().toUpperCase();
